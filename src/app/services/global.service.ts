...
export class GlobalService {
  ...
  getTestmonials(page){
    return this.http.get(`${environment.endpoint}/backend/ratings?page=${page}`)
  ...
  addToTestmonials(id){
    ...
    return this.http.post(`${environment.endpoint}/backend/ratings/in-home`,formData)
  ...
  getmessages(page){
    return this.http.get(`${environment.endpoint}/backend/contacts?page=${page}`)
  ...
  deleteNews(id){
    return this.http.get(`${environment.endpoint}/backend/news/delete?news_id=${id}`)
  }
  ...
  deleteAllShipmentTypes(id){
    return this.http.get(`${environment.endpoint}/backend/delete?shipment_type_id=${id}`)
  }

  editShipmentType(form){
    return this.http.post(`${environment.endpoint}/backend/shipment_types/edit`,form)
  }

  addShipmentType(form){
    return this.http.post(`${environment.endpoint}/backend/shipment_types/create`,form)
  }
...
getClients(active,page=1){
  return this.http.get(`${environment.endpoint}/backend/clients?active=${active}&page=${page}`)
...
getOrders(company_id ,status,changed=0,accept=0,canceled=0,page=1){

 return this.http.get(`${environment.endpoint}/backend/orders?company_id=1&status_id=${status}&is_changed=${changed}&accept_change=${accept}&canceled=${canceled}&page=${page}`)
}


getOrderspages(page = 1,company_id ,status,changed=0,accept=0,canceled=0){

  return this.http.get(`${environment.endpoint}/backend/orders?page=${page}&company_id=1&status_id=${status}&is_changed=${changed}&accept_change=${accept}&canceled=${canceled}`)
 }
...
updatePersonalInfo(f){
  ...
  formData.append('password',f.password)
  // formData.append('admin_id',f.admin_id)
  return this.http.post(`${environment.endpoint}/backend/update-profile`,formData);
}
updateAdmin(f){
  let formData = new FormData ()
...
getTransactions(page){
  return this.http.get(`${environment.endpoint}/backend/transactions?page=${page}`)
...
recieveOrder(id,note=''){
  let formData = new FormData ()
      formData.append('order_id',id)
  return this.http.post(`${environment.endpoint}/backend/orders/receive-china?note=${note}`, formData);
}
...
ChangeOrdersStatus(id,status,note=''){
  ...
  return this.http.post(`${environment.endpoint}/backend/orders/change-status?note=${note}`, formData);
...
//   return this.http.post(`${environment.endpoint}/backend/orders/cancel?note=${note}`, formData);
// }
    return this.http.post(`${environment.endpoint}/backend/orders/check-price?note=${note}`, formData);

}

personalData(id){
  return this.http.get(`${environment.endpoint}/backend/admins/show?admin_id=${id}`)
}
  //All orders
  allOrders(status_id){
